...
qvd2parquet 2.8.0  (c) 2026, RALFORION d.o.o.
...
symbol table says so before a row is read. A column whose symbols cannot fit
the dictionary page, or that has about as many distinct values as a row group
has rows, is written `PLAIN` from the start, raw bytes with only the
compressor working on it, rather than as a dictionary page that fills up,
indices for the rows it covered, and plain for the rest in every row group. A
Qlik composite primary key is exactly that column: one distinct value per row.
Every other column keeps its dictionary, whatever the compression.
...
when nothing in the header says `.` groups thousands. An inferred grouping is
held to a stricter test than a declared one. The grouping has to be exact —
first group one to three digits, every later one three — so `1.5` beside `15`
is still kept; a whole number has to match to the digit, so `2.000.000.000`
beside `2000000001` is kept; padding is re-tested once the separators are gone,
so `0.003.449` is the code `0003449` rather than a rendering of `3449`; and a
declared decimal separator is believed, which makes `3.449` beside `3449`
informative when the field states `DecSep=.`.
