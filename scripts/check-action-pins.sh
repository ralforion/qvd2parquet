...
# It also exits 0 for a tag that does not exist, so an empty result has to be
# treated as a failure by the caller rather than trusted.
resolve_tag() {
...
while IFS=: read -r file line body; do
    ...
    actual=$(resolve_tag "$repo" "$comment")
    if [ -z "$actual" ]; then
